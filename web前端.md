...
- setState()
	react需要将数据存在state状态管理器中，而改变试图则需要改变state中的数据。
	改变state中的数据不能直接使用对象属性的方法去改变，错误演示：this.state.name='xxx';react中提供了setState的方法去改变管理器中的数据，只有通过setState的方式去改变才会变更试图中的数据，正确的方式：this.setState({ name:xxx });
	
	1. setState()的方法中可以传入一个参数，这个参数即为当前的存储状态，好处是当获取当前存储中的数据时不必繁琐的通过this.state[prop]的方式去获取，而是指直接通过传入的参数获取； this.setState(prevState=>({ bol:!prevState.bol }));
